    ...
    show = cata gshow
             where gshow (Position i) = "Position " ++ show i
                   gshow (NamedPosition t n) = concat ["NamedPosition ", show t
                                                      , " ", show n ]
                   gshow (SourceName t n) = concat ["SourceName ", show t
                                                      , " ", show n ]
...
data Node a = Position Int -- ^A position in the row
            ...
            | SourceName Text (Maybe Int) -- ^The name of a source and the index of the corresponding `RowStore`
...
mkSourceName :: Text -> Expression
mkSourceName t = In $ SourceName t Nothing

mkKnownSourceName :: Text -> Int -> Expression
mkKnownSourceName t n = In $ SourceName t (Just n)
...
toFormula = para tf
    where tf :: RAlgebra Node Formula
          ...
          tf (SourceName n _ ) | T.all isAlphaNum n = n
                               | otherwise = T.concat ["@{", n, "}"]
...
getPositions = cata gp
    where
        ...
        gp (SourceName _ Nothing) = []
        gp (SourceName _ (Just n)) = [n]
        gp (Constant _) = []
        gp (Unary _ ps) = ps
