...
eval = para ev
    where
      ...
      ev (SourceName name Nothing) = return . mkError $ "Expresión con fuente desconocida: " `T.append` name
      ev (SourceName _ (Just n)) = return $ toField n
      ev (Constant f) = return f
      ev (Unary info (_, v)) = opU info <$> v
      ev (Binary info (_, v1) (_, v2)) = opB info <$> v1 <*> v2
      ev (Prefix info vs) = opP info <$> mapM snd vs
      ev (Cast ft vs) = convert ft <$> mapM snd vs
